...
- The trains board sorts all upcoming arrivals by time; each row shows the route bullet, direction, station name, and the minutes until arrival written like the MTA platform clocks - big bare numbers under a small "mins" column header (0 when the train is due). The grouped page shows each watch's arrivals as a row of rounded tiles led by a small "mins" label. Minutes count down locally between the 30-second gateway polls; if no poll succeeds for 90 seconds a stale-data warning appears, and after 5 minutes the board is replaced by the connection error state
- If a watch has a walk time to the platform set (entered in the gateway's web UI), hard catches are flagged with a small dot next to the countdown: yellow when you'd have to rush, red when the train can't be caught. Trains you can make at a normal pace, and watches without an estimate, show no dot
