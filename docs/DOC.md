...
- The trains board sorts all upcoming arrivals by time; each row shows the route bullet, direction, station name, and the minutes until arrival written like the MTA platform clocks - a big number with a small "min" underneath (0 when the train is due). The grouped page uses the same style in a row of tiles under each watch. Minutes count down locally between the 30-second gateway polls; if no poll succeeds for 90 seconds a stale-data warning appears, and after 5 minutes the board is replaced by the connection error state
